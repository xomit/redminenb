...
public final class IssueTimeTrackerTopComponent extends TopComponent {
	...
	private final Timer refreshTimer = new Timer(1000, (ActionEvent e) -> {
		refreshDisplay();
	});
	private boolean running;
	private Date start;
	private long savedTime;
	private transient RedmineIssue issue;
	...
	public void save() {
		...
		if (result == DialogDescriptor.OK_OPTION) {
			final TimeEntry te = new TimeEntry(issue.getRepository().getManager().getTransport());
	...
	private boolean checkIssueChange(RedmineIssue oldIssue, RedmineIssue newIssue) {
		if (running || savedTime > 0) {
			...
			DialogDescriptor dd = new DialogDescriptor( Bundle.MSG_Change_Running_Issue(oldIssue.getDisplayName()), Bundle.TTL_Change_Running_Issue());
