...
public class RedmineIssueController implements IssueController {
	...
	public RedmineIssueController(RedmineIssue issue) {
		...
		redmineIssue.addPropertyChangeListener((PropertyChangeEvent evt) -> {
	...
	private void initActions() {
		Action timeTrackingAction = new AbstractAction(Bundle.CTL_OpenIssueForTimeTracking(), Defaults.getIcon("appointment-new.png")) {
		...
		Action refreshAction = new AbstractAction(Bundle.CTL_RefreshAction(), Defaults.getIcon("refresh.png")) {
			@Override
			public void actionPerformed(ActionEvent e) {
				new SwingWorker() {
		...
		Action showInBrowserAction = new AbstractAction(Bundle.CTL_ShowInBrowserAction(), Defaults.getIcon("redmine.png")) {
			@Override
			public void actionPerformed(ActionEvent e) {
				try {
					URL url = URI.create(redmineIssue.getRepository().getUrl() + "/issues/" + redmineIssue.getID()).toURL(); // NOI18N
		...
		Action createSubTaskAction = new AbstractAction(Bundle.CTL_CreateSubTaskAction(), Defaults.getIcon("subtask.png")) {
			@Override
			public void actionPerformed(ActionEvent e) {
		...
		viewWatchers = new AbstractAction(Bundle.CTL_OpenWatchersList(), Defaults.getIcon("face-glasses.png")) {
